...
export default {
    ...
    async mounted(){
        var response = await $.callAPI(axios, 'GET', 'items/train_names?from_selecter=1&obj=1');
        if (response.http_status >= 400) return false;
        this.data_train_names = response.data;
    },
    methods:{
        changed(){
    ...
    <div v-if="value">
        <b-card body-class="p-1" @click="$refs._modal.show()">
            ...
            <div v-for="(item, i) in value" :key="i">
                ...
                <span class="small">{{(data_train_names[item.value]||{}).name_chi}}</span>
            </div>
        </b-card>
        <!-- Edit Modal -->
        <b-modal ref="_modal" hide-header hide-footer centered>
            <div class="row" v-for="(item, i) in value" :key="i">
                ...
                <div class="col-6">
                    <select-item type="train_name" v-model="value[i].value" size="sm" @input="changed()" />
                </div>
            </div>
            <div class="row">
                <div class="col-6">
                ...
                <div class="col-6">
                    <select-item type="train_name" v-model="new_value" size="sm" @change="add()" />
                </div>
            </div>
        </b-modal>
    </div>
